...
import (
	...
	"github.com/ibuilding-x/driver-box/internal/plugins/knx/knx/knxnet"
	"github.com/ibuilding-x/driver-box/internal/plugins/knx/knx/util"
	"net"
	"sync"
	"time"
)
...
func (conn *Tunnel) hostInfo() (knxnet.HostInfo, error) {
	...
	//fix docker problem
	if conn.config.LocalAddress != "" {
		uaddr, err := net.ResolveUDPAddr("udp4", conn.config.LocalAddress)
		if err == nil {
			addr = uaddr
		}
	}
